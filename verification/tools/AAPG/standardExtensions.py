...
def gen_sp_fp_instr(opcode):
	...
	if opcode in op.sp_fp_i+op.sp_fp_s:
		...
		instruction= '{0} {1}, {2}, {3}, {4} {5}'.format(opcode, reg_f_1, reg_f_2, reg_f_3, reg_f_4,roundingmode)
		
	elif opcode in ('fadd.s', 'fsub.s', 'fmul.s', 'fdiv.s'):
		instruction = '{0} {1}, {2}, {3} {4}'.format(opcode, reg_f_1, reg_f_2, reg_f_3,roundingmode)
	
	elif opcode in ('fsgnj.s', 'fsgnjn.s', 'fsgnjx.s', 'fmin.s', 'fmax.s'):
		instruction = '{0} {1}, {2}, {3}'.format(opcode, reg_f_1, reg_f_2, reg_f_3)
	
	elif opcode in('fsqrt.s'):
		instruction = '{0} {1}, {2} {3}'.format(opcode, reg_f_1, reg_f_2,roundingmode)
		
	elif opcode in ('fcvt.w.s', 'fcvt.wu.s', 'fcvt.l.s', 'fcvt.lu.s') :
		reg_int=find_rand_destination_reg()
		instruction= '{0} x{1}, {2} {3}'.format(opcode,reg_int,reg_f_1,roundingmode)
	
	elif opcode in ('fmv.x.s', 'fclass.s') :
		reg_int=find_rand_destination_reg()
	...
	elif opcode in ('fcvt.s.w', 'fcvt.s.wu', 'fcvt.s.l', 'fcvt.s.lu'):
		reg_int=find_rand_destination_reg()
		instruction= '{0} {1}, x{2} {3}'.format(opcode, reg_f_1, reg_int,roundingmode)		
	
	elif opcode in ('fmv.s.x'):
		reg_int=find_rand_destination_reg()
...
def gen_dp_fp_instr(opcode):
	...
	elif opcode in ('fadd.d', 'fsub.d', 'fmul.d', 'fdiv.d'):
		instruction = '{0} {1}, {2}, {3} {4}'.format(opcode, reg_f_1, reg_f_2, reg_f_3,roundingmode)
		
	elif opcode in ('fsgnj.d', 'fsgnjn.d', 'fsgnjx.d', 'fmin.d', 'fmax.d'):
		instruction = '{0} {1}, {2}, {3}'.format(opcode, reg_f_1, reg_f_2, reg_f_3)
	
	elif opcode in('fsqrt.d','fcvt.s.d','fcvt.d.s'):
		instruction = '{0} {1}, {2} {3}'.format(opcode, reg_f_1, reg_f_2,roundingmode)
		
	elif opcode in ('fcvt.w.d', 'fcvt.wu.d', 'fcvt.l.d', 'fcvt.lu.d') :
		reg_int=find_rand_destination_reg()
		instruction= '{0} x{1}, {2} {3}'.format(opcode,reg_int,reg_f_1,roundingmode)
	
	elif opcode in ('fmv.x.d', 'fclass.d') :
		reg_int=find_rand_destination_reg()
		instruction= '{0} x{1}, {2}'.format(opcode,reg_int,reg_f_1)
	...
	elif opcode in ('fcvt.d.w', 'fcvt.d.wu', 'fcvt.d.l', 'fcvt.d.lu'):
		reg_int=find_rand_destination_reg()
		instruction= '{0} {1}, x{2} {3}'.format(opcode, reg_f_1, reg_int,roundingmode)		
	
	elif opcode in ('fmv.d.x'):
		reg_int=find_rand_destination_reg()
		instruction= '{0} {1}, x{2}'.format(opcode, reg_f_1, reg_int)		
